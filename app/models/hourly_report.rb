  ...
  class << self
    ...
    def perform
      ...
      if last_report = HourlyReport.asc(:report_time).last.try(:report_time)
        ...
        result = QuarterlyReport.where(:report_time.gte => (last_report + 30.minutes)).map_reduce(map, reduce).finalize(finalize).out(inline: 1)
      else
        display 'Never been done before. This could take a bit.'
        result = QuarterlyReport.map_reduce(map, reduce).finalize(finalize).out(inline: 1)
      end
