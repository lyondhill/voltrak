class ThirtyMinuteReport
  ...
  class << self
    ...
    def perform
      display 'performing 30 min'
      if last_report = ThirtyMinuteReport.asc(:report_time).last.try(:report_time)
        display 'last aggrogate: ' + last_report.to_s
        result = FiveMinuteReport.where(:report_time.gte => (last_report + 15.minutes)).map_reduce(map, reduce).finalize(finalize).out(inline: 1)
      else
    ...
    def map
      %Q{
        function() {
          ...
          key.setMinutes ((parseInt(key.getMinutes() / 30)) * 30);
          key.setSeconds (0);
    ...
    def display(msg)
      puts "[#{Time.now}][ThirtyMinuteReport] - #{msg}"
    end
